...
function SoldListingCard({ listing }: { listing: PublicListing }) {
  ...
  return (
    <article className="min-w-[300px] snap-start overflow-hidden rounded-lg border border-white/10 bg-card text-card-foreground shadow-[0_22px_60px_rgba(0,0,0,0.28)] transition duration-300 hover:-translate-y-1 hover:border-primary/35 sm:min-w-0">
      <div className="relative aspect-[4/3] overflow-hidden bg-muted">
        {listing.primary_image?.image_url ? (
          <img
            ...
            loading="lazy"
            decoding="async"
            className="size-full object-contain object-center grayscale transition duration-500 hover:scale-105"
          />
        ) : (
        ...
        <img
          src="/sold-stamp.svg"
          alt={messages.inventory.recentlySold.soldBadge}
          loading="lazy"
          decoding="async"
          className="absolute right-2 top-2 w-24 rotate-2 opacity-95 drop-shadow-[0_8px_18px_rgba(0,0,0,0.32)]"
        />
    ...
    <PublicSection className="pt-8">
      <div className="grid gap-7">
        ...
        {!recentlySoldQuery.isLoading && !errorMessage && listings.length > 0 ? (
          <div className="public-scrollbar flex w-full snap-x gap-4 overflow-x-auto pb-3 sm:grid sm:grid-cols-[repeat(auto-fill,minmax(300px,340px))] sm:justify-start sm:overflow-visible sm:pb-0">
